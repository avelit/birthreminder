package birthreminder.domain.util;
...
public final class JSR310DateConverters {
    ...
    public static class ZonedDateTimeToLocalDateConverter implements Converter<ZonedDateTime, LocalDate> {
        public static final ZonedDateTimeToLocalDateConverter INSTANCE = new ZonedDateTimeToLocalDateConverter();
        private ZonedDateTimeToLocalDateConverter() {}

        @Override
        public LocalDate convert(ZonedDateTime source) {
            return source == null ? null : LocalDate.from(source.toInstant());
        }
    }

    public static class LocalDateToZonedDateTimeConverter implements Converter<LocalDate, ZonedDateTime> {
        public static final LocalDateToZonedDateTimeConverter INSTANCE = new LocalDateToZonedDateTimeConverter();
        private LocalDateToZonedDateTimeConverter() {}

        @Override
        public ZonedDateTime convert(LocalDate source) {
            return source == null ? null : ZonedDateTime.ofInstant(source.atStartOfDay(ZoneId.systemDefault()).toInstant(), ZoneId.systemDefault());
        }
    }
}
